...
-- SELECT first_name, COUNT(vehicles.id) AS count
--   FROM owners
--     JOIN vehicles
--       ON owner_id = owners.id
--         GROUP BY owners.id
...
-- SELECT first_name, ROUND(AVG(price), 0) AS average_price, COUNT(vehicles.id) AS count
--   FROM owners
--     JOIN vehicles
--       ON owner_id = owners.id
--         GROUP BY owners.id
--           HAVING COUNT(vehicles.id) > 1 AND ROUND(AVG(price), 0) > 10000
